...
export const MODULES = [
  ...
  },
  {
    ...
    lessons: [
      L('m4-l1', 'Python Syntax: A JS-to-Python Phrasebook', 'Variables, functions, loops — side-by-side with the JavaScript you write daily.', 35, 'Beginner', 90, { live: true, prereqs: ['m1-l2'] }),
      L('m4-l2', 'Lists, Dicts & Comprehensions', 'Python data structures vs arrays and objects — plus the one-liners Python devs love.', 35, 'Beginner', 90, { live: true, prereqs: ['m4-l1'] }),
      L('m4-l3', 'Functions, Classes & Modules', 'def, self, imports, and packages — organized like your JS projects.', 35, 'Beginner', 90, { live: true, prereqs: ['m4-l1'] }),
      L('m4-l4', 'pip, venv & Project Setup', 'npm-to-pip translation: environments, requirements.txt, and pyproject.', 25, 'Beginner', 80, { live: true, prereqs: ['m4-l1'] }),
      L('m4-l5', 'Async Python & Working with APIs', 'async/await you already know, now with httpx and the OpenAI/Anthropic SDKs.', 40, 'Intermediate', 110, { live: true, prereqs: ['m4-l3', 'm2-l1'] }),
      L('m4-l6', 'Notebooks & Quick Experiments', 'Jupyter as your AI scratchpad — when to use it and when to avoid it.', 25, 'Beginner', 80, { live: true, prereqs: ['m4-l1'] }),
      L('m4-l7', 'Checkpoint Project: Port a JS Tool to Python', 'Rebuild a small CLI tool in Python, calling an LLM API.', 60, 'Intermediate', 150, { live: true, prereqs: ['m4-l5'] }),
    ],
  },
  {
    ...
    lessons: [
      L('m5-l1', 'Embeddings: How Meaning Becomes Numbers', 'Words as coordinates in space — explore a live 2D embedding map.', 35, 'Intermediate', 110, { live: true, prereqs: ['m1-l2'] }),
      L('m5-l2', 'Attention: The Idea That Changed Everything', 'Watch tokens look at each other in an interactive attention visualizer.', 40, 'Intermediate', 120, { live: true, prereqs: ['m5-l1'] }),
      L('m5-l3', 'The Transformer Architecture Tour', 'Layers, heads, and the residual stream — a guided walkthrough diagram.', 40, 'Intermediate', 120, { live: true, prereqs: ['m5-l2'] }),
      L('m5-l4', 'Tokenizers Deep Dive', 'BPE explained by building one — why "strawberry" breaks models.', 35, 'Intermediate', 110, { live: true, prereqs: ['m1-l4'] }),
      L('m5-l5', 'Sampling Strategies: Greedy, Beam, top-k, top-p', 'Every decoding strategy raced side-by-side in a simulator.', 35, 'Intermediate', 110, { live: true, prereqs: ['m1-l2', 'm2-l5'] }),
      L('m5-l6', 'KV Cache & Why Long Context Is Expensive', 'The memory trick behind fast inference — and its costs.', 30, 'Advanced', 120, { live: true, prereqs: ['m5-l3'] }),
      L('m5-l7', 'Model Sizes, Scaling & Distillation', 'What 7B vs 70B actually means for quality, speed, and price.', 30, 'Intermediate', 100, { live: true, prereqs: ['m1-l3'] }),
      L('m5-l8', 'Checkpoint: Under-the-Hood Boss Quiz', 'Explain the machine like an engineer, not a user.', 20, 'Intermediate', 150, { live: true, prereqs: ['m5-l7'] }),
    ],
  },
  {
    ...
    lessons: [
      L('m6-l1', 'Embedding APIs & What They Encode', 'Generate real embeddings and measure similarity in code.', 30, 'Intermediate', 100, { live: true, prereqs: ['m5-l1'] }),
      L('m6-l2', 'Similarity Without Tears', 'Cosine similarity as "how aligned are two arrows" — no linear algebra degree needed.', 30, 'Intermediate', 100, { live: true, prereqs: ['m6-l1'] }),
      L('m6-l3', 'Vector Databases', 'Pinecone, pgvector, Chroma, Qdrant — how they index millions of vectors fast.', 35, 'Intermediate', 110, { live: true, prereqs: ['m6-l1'] }),
      L('m6-l4', 'Chunking Strategies', 'How you split documents makes or breaks retrieval quality.', 35, 'Intermediate', 110, { live: true, prereqs: ['m6-l1'] }),
      L('m6-l5', 'Hybrid Search & Reranking', 'Combining keyword + vector search, then reranking for precision.', 35, 'Advanced', 120, { live: true, prereqs: ['m6-l3'] }),
      L('m6-l6', 'Build: Semantic Search for Your Docs', 'A working search engine over markdown files, end to end.', 60, 'Intermediate', 140, { live: true, prereqs: ['m6-l4'] }),
      L('m6-l7', 'Checkpoint: Search Quality Challenge', 'Tune chunking + retrieval to beat a baseline relevance score.', 30, 'Advanced', 150, { live: true, prereqs: ['m6-l6'] }),
    ],
  },
  {
    ...
    lessons: [
      L('m7-l1', 'Why RAG Exists', 'Context injection vs fine-tuning vs bigger models — the decision tree.', 25, 'Intermediate', 100, { live: true, prereqs: ['m6-l1'] }),
      L('m7-l2', 'RAG Architecture End-to-End', 'Ingest → embed → store → retrieve → augment → generate, fully animated.', 35, 'Intermediate', 110, { live: true, prereqs: ['m7-l1', 'm6-l3'] }),
      L('m7-l3', 'Ingestion Pipelines', 'PDFs, HTML, and messy real-world documents into clean chunks.', 35, 'Intermediate', 110, { live: true, prereqs: ['m7-l2'] }),
      L('m7-l4', 'Retrieval Quality & RAG Evals', 'Measuring "did we fetch the right stuff" before blaming the model.', 35, 'Advanced', 120, { live: true, prereqs: ['m7-l2'] }),
      L('m7-l5', 'Citations & Grounded Answers', 'Making the model quote its sources — and admit when it doesn\'t know.', 30, 'Intermediate', 110, { live: true, prereqs: ['m7-l2'] }),
      L('m7-l6', 'Advanced RAG: Query Rewriting, Multi-hop, Agentic RAG', 'When naive retrieval fails and what to layer on top.', 40, 'Advanced', 130, { live: true, prereqs: ['m7-l2'] }),
      L('m7-l7', 'Production RAG: Cost, Latency & Caching', 'Serving RAG at scale without burning money.', 35, 'Advanced', 120, { live: true, prereqs: ['m7-l2'] }),
      L('m7-l8', 'Checkpoint Project: Chat With Your Docs', 'A cited docs-chatbot over content you choose — the classic AI engineer interview project.', 120, 'Advanced', 200, { live: true, prereqs: ['m7-l5', 'm7-l4'] }),
    ],
  },
  {
